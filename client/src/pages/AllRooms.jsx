...
const RaadioButton = ({label, selected = false, onChange = () => { }}) => {
    return (
        <label className='flex gap-3 items-center cursor-pointer mt-2 text-sm'>
...
const AllRooms = () => {
    ...
    const filteredRooms = useMemo(() => {
        return rooms.filter(room => matchesRoomType(room) && matchedPriceRange(room) && filterDestination(room)).sort(sortRooms);
        ...
        <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            ...
            <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
                ...
                <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-gray-300 ${openFilters && "border-b"}`}>
                    <p className='text-bse font-medium text-gray-800'>FILTERS</p>
                ...
                <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-700`}>
                    ...
                    <div className='px-5 pt-5 pb-7'>
                        ...
                        {sortOptions.map((option, index)=> (
                            <RaadioButton key={index} label={option} selected={selectedSort === option} onChange={() => handleSortChanges(option)}/>
                        ))}
                    </div>
                </div>
