...
const ListRoom = () => {
    ...
    // Toogle Availability of Room
    const toogleAvailability = async (roomId) => {
        const {data} = await axios.post('/api/rooms/toogle-availability', {roomId}, {headers: {Authorization: `Bearer ${await getToken()}`}});
                            ...
                            <tr key={index}>
                                ...
                                <td className='py-3 px-4 border-t border-gray-300 text-sm text-red-500 text-center'>
                                    <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                                        <input onChange={() => toogleAvailability(item._id)}
